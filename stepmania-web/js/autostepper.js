...
function generateBaseNotes(positions, difficulty, jumpProb, handsProb) {
  let filtered;
  ...
  for (const pos of filtered) {
    const cols = [0,0,0,0];
    let arr, tries = 0;
    do { arr = Math.floor(Math.random()*4); tries++; } while ((arr===last||arr===lastLast) && tries<8);
    cols[arr] = 1;
    if (Math.random() < realJumpProb) {
      let j; do { j = Math.floor(Math.random()*4); } while (j === arr);
      cols[j] = 1;
      if (Math.random() < realHandsProb) {
        let t; do { t = Math.floor(Math.random()*4); } while (cols[t] === 1);
        cols[t] = 1;
      }
    }
    notes.push({ pos, cols });
...
function addHoldsAndRolls(notes, difficulty, holdDensity) {
  ...
  for (let i = 0; i < sorted.length-1; i++) {
    ...
    const taps = [];
    for (let c = 0; c < 4; c++) if (cur.cols[c] === 1) taps.push(c);
    ...
    if (!endNote) { endNote = { pos: endPos, cols: [0,0,0,0] }; byPos.set(endPos, endNote); notes.push(endNote); }
...
function notesToRows(notes, totalUnits) {
  ...
  for (let m = 0; m < totalMeasures; m++) {
    ...
    for (let r = 0; r < sub; r++) {
      ...
      out += (cols ? cols.map(c => String(c)).join('') : '0000') + '\n';
    }
  }
  return out.trimEnd();
}
...
async function analyzeOne(q) {
  ...
  for (const d of diffs) {
    const baseNotes = generateBaseNotes(positions, d.key, jumpProb, 0.15);
    const finalNotes = addHoldsAndRolls(baseNotes, d.key, holdDensity);
    const rows = notesToRows(finalNotes, totalUnits);
    ...
    charts.push({ name: d.name, key: d.key, rating, notes: rows, count, radar });
  }
  q.result = { bpm, offsetSec, duration: buffer.duration, sampleStart, charts, sampleRate: sr };
  q.status = 'done';
  log(`  ✓ BPM ${bpm.toFixed(1)}, ${charts.reduce((s,c) => s+c.count, 0)} pasos`, 'ok');
  renderQueue();
}
...
async function saveAllToLibrary() {
  ...
  for (const q of done) {
    ...
    await dbAdd({
      ...
      charts: q.result.charts.map(c => ({ name: c.name, key: c.key, rating: c.rating, count: c.count })),
      addedAt: Date.now()
    });
    saved++;
